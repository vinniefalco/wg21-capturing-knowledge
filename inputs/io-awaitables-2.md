...
## 2. Evidence from the Ecosystem

This section documents ecosystem evidence motivating the need for a networking-specific execution model. The IoAwaitable protocol emerges from observed friction when applying general-purpose abstractions to I/O workloads.

### 2.1 Documented Integration Challenges

> [Evidence needed: Bug reports or GitHub issues where P2300's query model caused problems in networking contexts. Examples might include:
> - Issues with allocator timing in coroutine-based senders
> - Problems with stop token discovery in networking scenarios
> - Template instantiation explosion in I/O library wrappers]

### 2.2 Project Experience Reports

> [Evidence needed: Named projects that attempted P2300 for networking and encountered friction. Information to gather:
> - Project name and brief description
> - Specific pain points encountered
> - Workarounds employed or reasons for abandoning the approach]

### 2.3 Engineering Cost Quantification

> [Evidence needed: Measured metrics comparing approaches. Potential measurements:
> - Compile-time overhead: P2300 sender chains vs. IoAwaitable coroutines
> - Binary size impact: type-erased vs. fully-typed operation state
> - Development time: implementing equivalent functionality in both models]

### 2.4 Why This Evidence Matters

The architectural arguments in this paper (§7, §4.5) explain *why* P2300's backward query model conflicts with coroutine allocation semantics. This evidence section aims to demonstrate *that* the conflict manifests in practice. Strong evidence here would transform the argument from design-theoretical to evidence-based.

Until this evidence is gathered, readers should evaluate the architectural arguments on their merits while recognizing that field validation is ongoing through Boost.Capy and Boost.Corosio deployments.
...
### 3.0 Principle Importance Ranking

Not all requirements carry equal weight. Following D3004R0 principled design methodology, we characterize requirements by importance to enable principled tradeoff analysis:

**Imperative (@) — Absolute Requirements:**
- **Allocator discoverable before frame allocation.** Coroutine `operator new` executes before the coroutine body. Any mechanism that injects allocators later—receiver connection, `await_transform`—arrives too late. This is a timing constraint, not a design preference.

**High Priority (9):**
- **Forward context propagation.** Context flows from caller to callee at each suspension point. This aligns with coroutine semantics and eliminates late-binding problems.
- **Minimal executor abstraction.** Two operations (`dispatch` and `post`) suffice for I/O workloads. Additional complexity serves GPU dispatch, not networking.

**Medium Priority (5):**
- **Type erasure at API boundaries.** Concrete executor types should not leak into public interfaces. This enables ABI stability and reduces template instantiation.
- **Stop token propagation.** Cooperative cancellation must flow through the coroutine chain to reach I/O objects that can act on it.
- **Platform integration.** The model must accommodate IOCP, epoll, io_uring, and kqueue without artificial constraints.

**Lower Priority (1):**
- **Syntax convenience.** Clean `co_await` expressions are desirable but not essential; correctness and performance take precedence.
- **Interoperability with P2300.** While useful for mixed codebases, the primary design need not be constrained by sender/receiver compatibility.

This ranking informs the design decisions throughout this paper. When tradeoffs arise, higher-priority principles take precedence.
...
### 4.2 Satisfying _IoAwaitableTask_
...
> **Non-normative note.** The `io_awaitable_support` CRTP mixin (§9.1) provides all six required methods and is offered as a convenience. It is not proposed for standardization and is not strictly necessary—implementors may write the boilerplate themselves.


### 4.3 Satisfying _IoLaunchableTask_
...
### 7.0 Status Quo: Continue Without New Protocol

Before evaluating alternatives, we must consider the baseline: **what happens if we adopt no new protocol?**

**What this means:** Continue using existing approaches—Boost.Asio for production networking, ad-hoc coroutine wrappers for new projects, or wait for P2300 to mature and adapt networking to its model.

**Advantages of the status quo:**
- **No new learning curve.** Developers know Boost.Asio; no new concepts to master.
- **Proven stability.** Boost.Asio has decades of production deployment.
- **No standardization costs.** No committee time, no vendor implementation burden, no ecosystem disruption.
- **Flexibility.** Each project can choose its own approach without protocol constraints.

**Disadvantages of the status quo:**
- **No coroutine-first design.** Boost.Asio's coroutine support was retrofitted; the execution model predates C++20 coroutines.
- **Ecosystem fragmentation.** Without a protocol, coroutine-based I/O libraries cannot interoperate. Each framework defines its own task type, its own executor model, its own context propagation.
- **Repeated solutions.** Every library author re-discovers the same problems (allocator timing, stop token propagation, executor affinity) and implements ad-hoc solutions.
- **No vocabulary type.** There is no equivalent of `std::future` for coroutine-based I/O—no common currency for async operations.

**Why change may be justified:**

> [Evidence needed: Specific coordination failures or ecosystem fragmentation incidents that demonstrate the status quo's cost. See §2 for evidence gathering plan.]

The status quo is a valid choice. If the disadvantages above do not manifest as real-world problems—if library authors successfully build interoperable coroutine I/O without a common protocol—then the IoAwaitable proposal is unnecessary. This paper argues that the disadvantages are real, but acknowledges that field evidence is still being gathered.
...
### 8.2 Satisfying _IoAwaitableTask_
...
### 8.3 Satisfying _IoLaunchableTask_
...
Section 7 analyzes the costs of adopting P2300 for networking. For balance, this section examines the costs of the IoAwaitable proposal itself. Honest cost accounting requires evaluating both sides of the tradeoff.

### 10.1 Implementation Burden

**Vendor implementation costs (if standardized):**

| Component | Estimated Effort | Notes |
|-----------|------------------|-------|
| `executor_ref` | Low | Two-pointer type erasure wrapper |
| `execution_context` | Medium | Service registry with shutdown ordering |
| Concepts (`io_awaitable`, etc.) | Low | Concept definitions only |
| Launch functions | Medium | `run_async` two-call syntax is subtle |
| Thread-local allocator | Low | Single TLS pointer management |

**Comparison to P2300:** The IoAwaitable specification (§14) is substantially smaller than P2300. Fewer concepts, no domain machinery, no query protocol. However, "smaller" does not mean "trivial"—vendors must still implement, test, and maintain the components.

### 10.2 Interaction Surface

The proposal interacts with existing standard library components:

| Component | Interaction | Risk Level |
|-----------|-------------|------------|
| `<coroutine>` | Core dependency | Low — stable since C++20 |
| `<stop_token>` | Direct use of `std::stop_token` | Low — stable interface |
| `<memory_resource>` | `std::pmr::memory_resource*` for allocators | Low — stable interface |
| `<concepts>` | Concept definitions | Low — stable since C++20 |
| `<functional>` | None | N/A |
| `std::execution` | None required | Interop possible but not mandated |

The interaction surface is narrow by design. The protocol deliberately avoids coupling to `std::execution` to prevent inheriting its complexity costs.

### 10.3 Maintenance Commitment

**Expected maintenance burden:**

- **Defect reports:** Concept wording may require refinement as implementations discover edge cases.
- **Evolution:** Future coroutine language features (e.g., coroutine parameters) may require protocol updates.
- **Platform integration:** New OS async APIs (beyond IOCP/epoll/io_uring) may surface design gaps.

**Mitigation:** The protocol's minimal surface area limits exposure. Most evolution happens in the I/O libraries built atop the protocol, not in the protocol itself.

### 10.4 Teaching and Documentation

**Learning curve:**
- **For library authors:** Understanding the three concepts, the `await_suspend` signature, and TLS allocator propagation. Estimated: 1-2 days for experienced developers.
- **For library users:** Minimal—users write `co_await` expressions and provide executors at launch sites. The protocol machinery is invisible.

**Documentation burden:**
- Concept requirements documentation
- Launch function usage examples
- Migration guide from Boost.Asio patterns

### 10.5 Cost Comparison Summary

| Cost Category | IoAwaitable | P2300 (for networking) |
|---------------|-------------|------------------------|
| Specification size | ~500 lines | ~3000+ lines |
| Concept count | 4 | 15+ |
| Vendor implementation | Medium | High |
| Interaction surface | Narrow | Wide (domains, queries, transforms) |
| Teaching burden | Low | High |
| Runtime overhead | ~1-2ns type erasure | Variable (depends on customization) |

This comparison is not neutral—the authors obviously favor IoAwaitable. Readers should evaluate whether the cost differential justifies a separate protocol or whether ecosystem unification under P2300 is worth the complexity.
...
7. **Safety contribution**: The protocol contributes to C++ safety objectives in several ways:
   - **Memory safety**: The TLS allocator mechanism ensures all coroutine frames in a chain use consistent allocation, preventing mixed-allocator bugs. Frame lifetime is tied to coroutine lifetime, reducing dangling pointer risks.
   - **Type safety**: The concept constraints (`io_awaitable`, `io_awaitable_task`, `io_launchable_task`) provide compile-time verification that types conform to protocol requirements.
   - **Thread safety**: Executor affinity ensures coroutines resume on expected execution contexts, preventing data races from unexpected thread transitions.

8. **ABI stability**: The type-erased design explicitly supports ABI stability:
   - **`executor_ref`** is a two-pointer wrapper with stable layout regardless of the underlying executor type.
   - **Public interfaces** expose only `executor_ref`, not concrete executor types, allowing implementation changes without breaking binary compatibility.
   - **No template proliferation** at API boundaries means shared libraries can export coroutine-based APIs.
   - **Forward compatibility**: New executor types can be added without recompiling dependent code.

The comparison with `std::execution` (§3.4, §7) is instructive: that framework's complexity serves GPU workloads, not networking. The relatively large number of related papers suggests a design that is not yet mature and still finding its footing. P3826 adds machinery to fix problems networking doesn't have: domain-based algorithm dispatch, completion scheduler queries, sender transforms. Our design sidesteps these issues entirely.
...
## 12. Retrospective Commitment

This section defines how we will measure success and when we will revisit the design. A protocol without accountability is a protocol that cannot learn from its mistakes.

### 12.1 Success Criteria

The IoAwaitable protocol succeeds if:

| Criterion | Measurement | Target |
|-----------|-------------|--------|
| **Adoption breadth** | Number of libraries implementing the protocol | ≥3 independent libraries within 3 years |
| **Production deployment** | Organizations using protocol-based libraries in production | ≥5 documented deployments within 3 years |
| **Interoperability validation** | Demonstrated interop between independently developed libraries | At least 2 libraries successfully interoperating |
| **Performance parity** | Benchmark comparison with hand-written coroutine code | ≤5% overhead vs. manual implementation |
| **Developer satisfaction** | Survey or testimonial evidence | Net positive feedback from adopters |

### 12.2 Failure Indicators

The protocol design should be revisited if:

- **Adoption stalls:** Fewer than 2 libraries adopt within 2 years despite awareness.
- **Interop fails:** Libraries claim protocol compliance but fail to interoperate due to specification ambiguity.
- **Performance problems:** The type-erasure overhead proves significant for real workloads (not microbenchmarks).
- **Allocator mechanism rejected:** The TLS-based allocator propagation proves unworkable in practice.
- **P2300 succeeds for networking:** If P2300 evolution (P3826+) adequately addresses networking concerns, a separate protocol may be unnecessary.

### 12.3 Retrospective Schedule

| Milestone | Timing | Action |
|-----------|--------|--------|
| **Initial review** | 1 year after Boost acceptance | Gather early adopter feedback; identify specification gaps |
| **Mid-term assessment** | 2 years | Evaluate against success criteria; decide whether to pursue WG21 |
| **Full retrospective** | 3 years | Comprehensive evaluation; publish findings regardless of outcome |

### 12.4 Commitment

The authors commit to:

1. **Publishing retrospective findings** even if they indicate design failures.
2. **Maintaining the reference implementation** (Boost.Capy) for the evaluation period.
3. **Engaging with adopters** to gather feedback and address issues.
4. **Revising the protocol** if field experience reveals fundamental problems.

This commitment transforms the proposal from "trust our design" to "verify our design." The C++ community deserves evidence-based standardization.
...
### 13.1 External Validation

The following evidence would strengthen the case for this protocol:

**Production Deployments**

> [Evidence needed: Organizations using Boost.Capy or Boost.Corosio in production. Information to document:
> - Organization name (or anonymized description)
> - Use case (networking server, embedded system, etc.)
> - Scale (connections handled, throughput achieved)
> - Duration in production]

**User Feedback**

> [Evidence needed: Developer testimonials or survey results. Questions to address:
> - Does the protocol simplify coroutine-based I/O development?
> - Are the concepts intuitive or confusing?
> - What pain points remain?
> - Would you recommend this approach to others?]

**Adoption Metrics**

> [Evidence needed: Quantitative adoption data:
> - GitHub stars / forks for reference implementations
> - Download counts from package managers
> - Number of dependent projects
> - Conference talks or blog posts referencing the protocol]

### 13.2 Timeline and Milestones

The following roadmap guides the protocol's development toward potential standardization:

| Phase | Timeline | Milestone | Decision Point |
|-------|----------|-----------|----------------|
| **Boost Review** | 2026 Q2-Q3 | Submit Boost.Capy/Corosio for formal Boost review | Pass/fail determines continuation |
| **Field Experience** | 2026-2028 | Gather production usage data and feedback | Sufficient adoption to justify standardization? |
| **Evidence Assessment** | 2028 Q1 | Evaluate §2 evidence gaps and §12 success criteria | Is evidence sufficient for WG21? |
| **WG21 Decision** | 2028 Q2 | Decide whether to submit to WG21 | Submit, revise, or abandon standardization path |
| **P2300 Monitoring** | Ongoing | Track P3826 and networking evolution in std::execution | Does P2300 adequately serve networking? |

**Relationship to P2300/P3826:**

This proposal does not seek to block or replace `std::execution`. The frameworks serve different primary use cases:

- **P2300/P3826**: GPU dispatch, parallel algorithms, heterogeneous computing
- **IoAwaitable**: CPU-bound I/O, networking, coroutine-first design

If P3826 evolution adequately addresses networking concerns—particularly allocator timing and forward context propagation—the IoAwaitable protocol may become unnecessary. We will monitor this evolution and adjust our roadmap accordingly.
...
### 14.1 Header `<io_awaitable>` synopsis [ioawait.syn]
...
### 14.2 Concepts [ioawait.concepts]

#### 14.2.1 Concept `io_awaitable` [ioawait.concepts.awaitable]
...
#### 14.2.2 Concept `io_awaitable_task` [ioawait.concepts.task]
...
#### 14.2.3 Concept `io_launchable_task` [ioawait.concepts.launch]
...
#### 14.2.4 Concept `executor` [ioawait.concepts.executor]
...
### 14.3 Class `executor_ref` [ioawait.execref]
...
### 14.4 Class `execution_context` [ioawait.execctx]
...
### 14.5 Launch functions [ioawait.launch]
...
### 14.6 Namespace `this_coro` [ioawait.thiscoro]
